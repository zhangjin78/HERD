...
if [[ $# -ne 12 ]]; then
  echo "Usage: $0 PROC_ID TOTAL_EVENTS EVENTS_PER_JOB BASE_SEED OUTPUT_DIR ENERGY_MIN ENERGY_MAX PARTICLE GEOMETRY G4MAC TAG RUN_BASE" >&2
  exit 64
fi
...
output_name="${particle}_${energy_label}_vertical_job${job_index}_nevt${nevents}_seed${seed}.root"
