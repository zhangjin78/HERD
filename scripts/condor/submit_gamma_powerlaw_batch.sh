...
runner="${script_dir}/run_gamma_powerlaw_subjob.sh"
...
usage() {
  cat <<'EOF'
Usage:
  submit_gamma_powerlaw_batch.sh --total-events N --events-per-job N --tag TAG [options]
...
Options:
  ...
  --spectrum TYPE         mono|powerlaw. Default: mono
                          powerlaw uses the HERDOS index alpha=-1 macro.
...
Examples:
  ...
  submit_gamma_powerlaw_batch.sh --total-events 1000000 --events-per-job 10000 \
...
case "$spectrum" in
  mono|powerlaw) ;;
  *) echo "ERROR: --spectrum must be mono or powerlaw." >&2; exit 69 ;;
esac
...
if [[ "$spectrum" == powerlaw && "$energy_min" == "$energy_max" ]]; then
  echo "ERROR: powerlaw spectrum requires --energy-range A B." >&2
  exit 69
fi
